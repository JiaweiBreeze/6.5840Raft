...
// leader to send self log to followers
// called in heartbeat and leader election
func (rf *Raft) appendEntries(heartBeat bool) {
	...
	for peer, _ := range rf.peers {
		...
		if lastLog.Index >= rf.nextIndex[peer] || heartBeat {
			...
			// rf construct method append a empty entry with
			//index 0 to log
			// leader change nextindex to lastlog.index + 1 in election
			if nextIndex <= 0 {
				nextIndex = 1
			}

			// confirm lastlog.idx - nextidx + 1 > 0
			...
			args := AppendEntriesArgs{
				...
				Entries:      make([]Entry, rf.log.lastLog().Index-nextIndex+1),
			}
...
func (rf *Raft) leaderSendEntries(serverId int, args *AppendEntriesArgs) {
	...
	// means other node has newer term than self
